...
This package emits composite-trust-query responses pinned to `canon_version: jcs-rfc8785-v1` on every emitted response. The pin is in-band; downstream consumers (including [`@algovoi/audit-verifier`](https://www.npmjs.com/package/@algovoi/audit-verifier) and any conformant third-party verifier) read the pin to select the canonicalisation rule applied at emission.
